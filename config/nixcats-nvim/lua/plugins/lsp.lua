...
local function detect_yaml_type(filepath, filename)
	if filename:match("azure%-pipelines%.ya?ml$") or filename:match("%.azure%-pipelines%.ya?ml$") then
	...
	if filename:match("values%.ya?ml$") or filepath:match("/templates/") then
		return "helm"
	end
